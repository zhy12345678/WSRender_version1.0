...
def reachable_ws_indices(Robot, QS, Joint_Flag, **kwargs):
    ...
    if opt['path'] is None:
        ...
        filename = opt['path']
        path = f"../Data_Dex/{filename}{Count}.csv"
    ...
    if opt['evaluate'] == 'Off':
        ...
        for i in range(Count):
            ...
            for K in range(Cal_Index):
                ...
                # or Num_Array[K] == Dex.shape[1]
                if Num_Array[K]> Dex.shape[1] :
                    # Placeholder column
                    num_rows = Dex.shape[0]
                    placeholder_column = np.zeros((num_rows, 1))  # Initialize with zeros

                    # Add placeholder column to the existing array
                    Dex = np.hstack((Dex, placeholder_column))

                Dex[i, Num_Array[K]-1] = local_evaluation(Indice_Group[K], N_DoF, Y, m, M, R[i, 0])
